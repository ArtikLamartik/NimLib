...
extern void*       condthr     (void);
extern void        cpfile      (const char* source, const char* destination);
extern void        cpfolder    (const char* source, const char* destination);
...
extern Lf          lf          (void);
extern void        lockthr     (void* mutex);
extern const char* lowstr      (const char* text);
...
extern double      randflt     (double minimum, double maximum);
...
extern const char* repstr      (const char* text, long count);
extern void        resetbgfg   (void);
extern const char* revstr      (const char* text);
...
extern Search      search      (const char* text, const char* pattern);
...
extern void        sigthr      (void* cond);
extern const char* sockacc     (void);
extern void        sockclose   (void);
extern void        sockcon     (const char* ipaddress, long port);
extern void        sockdiscon  (void);
extern const char* sockopen    (long port);
extern const char* sockrecv    (void);
...
extern void        stdo        (const char* text);
extern long        strcomp     (const char* text1, const char* text2);
extern long        strcount    (const char* text, const char* pattern);
extern const char* strdel      (const char* text, long index);
extern const char* strformat   (long count, ...);
extern const char* strinsert   (const char* text1, const char* text2, long index);
extern const char* substr      (const char* text, long start_index, long stop_index);
extern void        syncthr     (void (*function)(void));
extern const char* timeformat  (double timestamp, const char* format);
...
extern const char* tohexstr    (const char* value);
...
extern const char* tostrhex    (const char* value);
extern const char* tostrint    (long value);
extern const char* trimstr     (const char* text, long sides, const char* pattern);
extern void        undef       (const char* name);
extern void        unlockthr   (void* mutex);
extern void        until       (double timestamp);
extern const char* upstr       (const char* text);
extern const char* version     (void);
extern void        vidbuf      (long mode);
extern void        wait        (long milliseconds);
extern void        waithr      (void* cond, void* mutex);
